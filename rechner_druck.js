...
function listeDrucken() {
    let daten = entriesNoelke; if(daten.length === 0) { showToast("Liste ist leer!", "warning"); return; }
    
    let einheit = "E2";
    const config = getLeergutConfig();
    if(config.length > 0) einheit = config[0].name;

    let sortierteDaten = [...daten].sort((a, b) => a.prod.localeCompare(b.prod));
    let printHtml = "<div style='padding: 5mm; font-family: Arial, sans-serif; background: white; color: black;'><h2 style='text-align:center; color:black; margin-top:0;'>Tresch & Sohn - Nölke Lieferschein</h2><p style='text-align:center; color:black;'>Datum: " + new Date().toLocaleDateString('de-DE') + "</p><hr style='border: 1px solid black;'>";
    let aktuellesProdukt = "";
    sortierteDaten.forEach(item => { if(aktuellesProdukt !== item.prod) { printHtml += `<div style="font-weight: bold; font-size: 18px; margin-top: 15px; margin-bottom: 5px; border-bottom: 1px solid #000; color: black;">${item.prod}</div>`; aktuellesProdukt = item.prod; } printHtml += `<div style="font-size: 16px; margin-left: 10px; margin-bottom: 3px; color: black;">${item.mhd} &nbsp;&nbsp;&nbsp; ${item.e2} ${einheit} &nbsp;&nbsp;&nbsp; ${item.kg.toFixed(2).replace('.',',')} kg</div>`; });
    printHtml += "</div>";
    document.getElementById("printArea").innerHTML = printHtml; 
    setTimeout(() => { if (typeof forcePrint === 'function') { forcePrint('Noelke_Liste'); } else { window.print(); } }, 400);
}
...
function sortierungDrucken() {
    ...
    let printHtml = `<style>#printArea table { width: 100%; border-collapse: collapse; table-layout: fixed; } #printArea td, #printArea th { border: 2px solid black; padding: 6px 10px; font-family: Arial, sans-serif; font-size: 15px; text-align: left; color: black; } #printArea .center-text { text-align: center !important; }</style>`;
    ...
    document.getElementById("printArea").innerHTML = printHtml;
    setTimeout(() => { if (typeof forcePrint === 'function') { forcePrint('Paletten_Wiegeschein'); } else { window.print(); } }, 400);
}