...
function finalizePrintReklamation(datum, zeit, lief, ls, temp, mhd, worker, kategorie, soll, ist, leergutSoll, leergutIst, positionen, bemerkung, photoBase64) {
    ...
    printArea.innerHTML = generateOfficialBlock("Kopie Spedition / Fahrer") + "<div style='page-break-after: always;'></div>" + generateOfficialBlock("Kopie Tresch & Sohn");
    ...
    fetch(rekUrl + ".json", { method: 'POST', body: JSON.stringify(rekData), headers: { 'Content-Type': 'application/json' } })
    .then(() => {
    ...
    setTimeout(() => { if (typeof forcePrint === 'function') { forcePrint('Reklamation_' + lief.replace(/\s+/g, '_')); } else { window.print(); } }, 500);
}