...
function printLgZaehler() {
    ...
    setTimeout(() => {
        if (typeof forcePrint === 'function') {
            forcePrint('Leergut_Beleg' + safeName, printHtml);
        } else if (typeof printCleanDocument === 'function') {
            printCleanDocument({ title: 'Leergut-Beleg', subtitle: kunde, bodyHtml: printHtml });
        }
    }, 100);
}

function exportLeergutExcel() {
    const kunde = document.getElementById('lg-kunde').value.trim() || 'Unbekannt';
    const config = getLeergutConfig().filter(lg => !lg.name.toLowerCase().includes('herta'));
    const rows = [];
    config.forEach(lg => {
        const val = lgZaehlerState[lg.id] || { entladen: 0, geladen: 0 };
        if (val.entladen > 0 || val.geladen > 0) {
            rows.push({ "Kunde": kunde, "Leergut-Sorte": lg.name, "Entladen": val.entladen || 0, "Geladen": val.geladen || 0 });
        }
    });
    if (rows.length === 0) { showToast("Kein Leergut gezählt!", "warning"); return; }
    if (typeof downloadExcelSheet === 'function') {
        downloadExcelSheet(rows, 'Leergut', 'Leergut_' + kunde.replace(/[^a-zA-Z0-9]/g, '_') + '.xlsx');
