...
function printAnalysisTable() {
    const days = getDatesForWeekOffset(currentWeekOffset);
        ...
        days.forEach(d => {
            ...
            deliveries.forEach(del => {
                (del.workerShares || []).forEach(ws => {
                    ...
                    if (ws.name === w && wsDate === d) dayKg += ws.kg;
                });
            });
            const dayFactor = (dailyAttendance[d] && dailyAttendance[d][w]) ? dailyAttendance[d][w] : 0;
            workerTotalKg += dayKg;
    ...
    const title = 'Mitarbeiter Leistung KW ' + weekNum + ' (' + firstDayObj.getFullYear() + ')';
    if (typeof printTableDocument === 'function') {
        printTableDocument({ title, subtitle: 'Tresch & Sohn Logistik', headers, rows, landscape: true });
    } else if (typeof forcePrint === 'function') {
        let body = '<table><thead><tr>' + headers.map(h => '<th>' + h + '</th>').join('') + '</tr></thead><tbody>';
        rows.forEach(r => { body += '<tr>' + r.map(c => '<td>' + c + '</td>').join('') + '</tr>'; });
        forcePrint('Logistik_KW' + weekNum, body + '</tbody></table>');
    }
}
