let suppliers = [], customers = [], workers = [], deliveries = [], dailyStaff = {}, dailyAttendance = {}, teamTagesMengen = {}, workerColors = {}, articles = [], curId = null;
let supplierLines = {}, deletedSuppliers = [], supplierLinesCleared = [];
...
function loadLocalDB() {
    const val = ladeLogistikDb();
    suppliers = val.suppliers || []; 
    customers = val.customers || [];
    supplierLines = typeof purgeClearedSupplierLines === 'function'
        ? purgeClearedSupplierLines(val.supplierLines || {}, val.supplierLinesCleared)
        : (typeof normalizeSupplierLinesForFirebase === 'function'
            ? normalizeSupplierLinesForFirebase(val.supplierLines || {})
            : (val.supplierLines || {}));
    supplierLinesCleared = Array.isArray(val.supplierLinesCleared) ? val.supplierLinesCleared : [];
    ...
    const sortierBuchungen = val.teamSortierBuchungen || [];
    if (typeof syncSortierenTagessummeZuDeliveries === 'function') {
        deliveries = syncSortierenTagessummeZuDeliveries(
            deliveries,
            sortierBuchungen,
            val.teamTagesMengen || {},
            val.deletedSortierBuchungen,
            val.articles || [],
            val.artikelMarkt || {}
        );
    }
    ...
    teamTagesMengen = val.teamTagesMengen || {};
    migriereTeamAusDailyStaff(teamTagesMengen, dailyStaff);
    syncDailyStaffAusTeam(teamTagesMengen, dailyStaff);
...
function saveLocalDB() { 
    ...
    val.supplierLinesCleared = supplierLinesCleared;
    ...
    deliveries.forEach((d) => syncTeamTagesGesamtKg(teamTagesMengen, deliveries, d.date));
    val.teamTagesMengen = teamTagesMengen;
...
async function backupToCloud() {
    ...
    let combinedData = typeof getLogistikFullCloudPayload === 'function' ? getLogistikFullCloudPayload() : {};
    if (!requireAppAuth()) { if(btn) { btn.innerHTML = '<span style="font-size:20px;">☁️</span> In Cloud sichern'; btn.disabled = false; } return; }
    try {
        ...
        if (res.ok) {
            ...
            if (cloud && typeof mergeLogistikPayloadMitCloud === 'function') {
                combinedData = mergeLogistikPayloadMitCloud(combinedData, cloud);
            }
        }
    } catch (e) { /* offline */ }
    const patchData = typeof safeLogistikCloudPatchPayload === 'function' ? safeLogistikCloudPatchPayload(combinedData) : combinedData;
 ...
 */
async function manualCloudPush(btn) {
    if(btn) { 
        btn.innerHTML = "⏳"; 
        btn.disabled = true; 
    }
    ...
    saveLocalDB();
    let combinedData = typeof getLogistikFullCloudPayload === 'function' ? getLogistikFullCloudPayload() : {};
    if (!requireAppAuth()) { if(btn) { btn.innerHTML = '☁️'; btn.disabled = false; } return; }
    try {
        const res = await cloudFetch(APP_CONFIG.CLOUD_URL + ".json?t=" + Date.now());
        if (res.ok) {
            const cloud = await res.json();
...
function restoreFromCloud() {
    ...
    if (!requireAppAuth()) { if(btn) { btn.innerHTML = '<span style="font-size:20px;">🔄</span> Aus Cloud laden'; btn.disabled = false; } return; }
...
function getSupplierLinesList(name) {
    if (typeof isSupplierLinesCleared === 'function' && isSupplierLinesCleared(supplierLinesCleared, name)) return [];
    const lines = typeof lookupSupplierLines === 'function'
        ? lookupSupplierLines(supplierLines, name, supplierLinesCleared)
        : supplierLines[name];
...
function renderApp1() {
    ...
    if (document.getElementById('pageBackup') && document.getElementById('pageBackup').classList.contains('active') && typeof renderAppCloudLogs === 'function') {
        renderAppCloudLogs();
    }
}
...
function deleteSupplier(name) {
    ...
    const lineKey = typeof firebaseSafeKey === 'function' ? firebaseSafeKey(name) : name;
    if (supplierLines[lineKey]) delete supplierLines[lineKey];
...
function saveDelivery() {
    ...
    if(k && s && d) {
        ...
        const entry = { id, date: d, name: s, kg: k, workerShares, source: 'lkw' };
...
function loadCloudReklamationen() {
    ...
    if (!requireAppAuth()) { list.innerHTML = "🔒 Bitte zuerst entsperren."; return; }
    cloudFetch(rekUrl + ".json?t=" + Date.now())
        .then(res => res.json())
        .then(data => {
...
function updateCloudReklamation() {
    ...
    const doUpload = (photoBase64) => {
        ...
        cloudFetch(rekUrl, { method: 'PUT', body: JSON.stringify(rekData), headers: { 'Content-Type': 'application/json' } }).then(res => {
...
function deleteCloudReklamation(key) {
    customConfirm("Möchtest du diese Reklamation wirklich dauerhaft aus der Cloud löschen?", () => {
        ...
        cloudFetch(rekUrl + "/" + key + ".json", { method: 'DELETE' })
            .then(res => {
                if (res.ok) {
