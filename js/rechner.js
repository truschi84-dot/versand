...
function resetBrandenburgForm() {
    ...
    if(istNettoEl) { istNettoEl.readOnly = true; istNettoEl.style.color = ""; istNettoEl.style.background = isDark ? 'var(--bg-disabled, #2c2c2c)' : 'var(--bg-disabled, #f0f2f5)'; }
    const leergutInput = document.getElementById('bb-leergut');
    if (leergutInput) { leergutInput.readOnly = true; leergutInput.style.background = isDark ? 'var(--bg-disabled, #2c2c2c)' : 'var(--bg-disabled, #f0f2f5)'; }
    brandenburgState.scans = [];
    ...
    document.getElementById('brandenburg-data-form').style.display = 'block';
    showToast("Eingabe zurückgesetzt", "success");
}
