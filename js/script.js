...
function getOtaConfigUrl() {
    if (location.protocol === 'https:' || location.protocol === 'http:') {
        ...
        return location.origin + dir + '/app-update.json';
    }
...
function isPinVersionRevoked(settings) {
    ...
    // Nur bei echter PIN-Änderung (Version in Cloud höher) — nicht bei veraltetem Backup oder Public-File-Abweichung
...
/** APK: nach OTA-Sprung auf GitHub wieder lokale Assets nutzen (eigener Storage!). */
function ensureNativeApkUsesBundledAssets() {
    if (typeof AndroidApp === 'undefined') return false;
    const href = String(location.href || '');
    if (/android_asset\/index\.html/i.test(href)) return false;
    const v = typeof getRunningWebVersion === 'function' ? getRunningWebVersion() : '';
    location.replace('file:///android_asset/index.html' + (v ? '?v=' + v : ''));
    return true;
}
...
window.onload = async () => {
    ...
    (async () => {
        ...
        if (isAppAuthenticated()) {
            ...
            resetInactivityTimer();
            startPinVersionWatch();
            verifySessionStillValid().then((ok) => {
                if (!ok && isAppAuthenticated()) {
                    /* revokeSessionIfPinChanged hat bereits entsperrt */
                }
            });
        } else {
            showPinProtection();
...
async function fetchOfficeAppMeta(base) {
    try {
        const v = await fetchJsonNoCache(normalizeOtaBase(base) + '/app-version.json?t=' + Date.now());
        return {
            webVersion: Number(v?.webVersion || 0),
            officeBundleId: String(v?.officeBundleId || ''),
...
function getStoredOfficeBundleId() {
    return String(AppStorage.getRaw('installed_office_bundle_id') || '');
}

function recordOfficeBundleId(bundleId) {
    const id = String(bundleId || '').trim();
    if (id) AppStorage.setRaw('installed_office_bundle_id', id);
}

async function recordOfficeBundleFromCurrentOrigin() {
    if (location.protocol !== 'http:' && location.protocol !== 'https:') return;
    const meta = await fetchOfficeAppMeta(location.origin);
    if (meta?.officeBundleId) recordOfficeBundleId(meta.officeBundleId);
}

async function officeHasNewerBundle(officeBase) {
    const office = normalizeOtaBase(officeBase);
    if (!office || !await probeOtaBase(office)) return null;
    const meta = await fetchOfficeAppMeta(office);
    if (!meta?.officeBundleId) return null;
    if (meta.officeBundleId === getStoredOfficeBundleId()) return null;
    return meta;
}
...
async function initOtaApkBootstrap() {
    ...
    try {
        ...
        // Nur auf Web wechseln wenn dort eine NEUERE Version liegt — nicht bei gespeicherter alter OTA-URL (sonst USB-Build übersprungen)
        const shouldUseWeb = remoteVer > getRunningWebVersion();
...
function initOtaUpdateWatch() {
    ...
    if (location.protocol === 'https:' || location.protocol === 'http:') {
        ...
        recordOfficeBundleFromCurrentOrigin().catch(() => {});
    }
}
...
/** Menü: App direkt vom Büro-Laptop im WLAN laden (ohne Versions-Vergleich). */
async function loadWlanUpdateNow() {
    if (typeof toggleMenuApp2 === 'function') toggleMenuApp2(false);
    if (!navigator.onLine) {
        showToast('Kein Netz – bitte WLAN prüfen.', 'error');
        return;
    }
    const saved = AppStorage.getRaw('last_office_lan_url');
    const { office, web } = await getOtaConfigBases();
    let base = await pickBestOtaInstallBase(web, office);
    if (!base && saved) base = normalizeOtaBase(saved);
    if (!base) {
        showToast('Büro-Server nicht erreichbar – Control Center auf dem Laptop starten.', 'error');
        return;
    }
    AppStorage.remove('ota_banner_dismissed');
    AppStorage.setRaw('ota_web_base_url', base);
    showToast('Lade vom Büro-Server …', 'info');
    navigateToOtaUrl(base + '/index.html?t=' + Date.now());
}
window.loadWlanUpdateNow = loadWlanUpdateNow;
...
async function applyOtaUpdate(remoteVer, remoteBase, webBase) {
    ...
    if (!installBase) {
        ...
        showOtaInfoModal(
            ...
            'Weder GitHub noch Büro-Server erreichbar.<br>Bitte Internet/WLAN prüfen – Test-Server: <code>scripts\\Control-Center-TEST.bat</code> (Port 8081).',
            '⚠️'
        );
        return;
    }
...
/** Gleiche Versionsnummer, aber Büro-Server erreichbar — Reload anbieten. */
function showOtaUpToDateWithOfficeOption(installedVer, officeBase) {
    const modal = document.getElementById('ota-update-modal');
    if (!modal) {
        showOtaInfoModal('App ist aktuell', 'Version <strong>v' + installedVer + '</strong>.', '✓');
        return;
    }
    restoreOtaUpdateModalLayout();
    const h2 = modal.querySelector('h2');
    ...
    const iconEl = modal.querySelector('.ota-update-icon');
    const office = normalizeOtaBase(officeBase);
    if (h2) h2.textContent = 'Version v' + installedVer + ' — synchron';
    if (iconEl) iconEl.textContent = '✓';
    if (textEl) {
        textEl.innerHTML = 'Handy und Laptop haben denselben Stand (v' + installedVer + ').'
            + '<br><br>Trotzdem neu laden: <strong>Vom Büro-Server laden</strong>.';
    }
    if (confirmBtn) {
        confirmBtn.textContent = 'Vom Büro-Server laden';
        confirmBtn.style.background = '#25D366';
        confirmBtn.disabled = false;
        confirmBtn.onclick = () => {
            hideOtaUpdateModal();
            AppStorage.setRaw('ota_web_base_url', office);
            navigateToOtaUrl(office + '/index.html?t=' + Date.now());
        };
    }
    if (laterBtn) {
        laterBtn.textContent = 'Schließen';
        laterBtn.style.display = '';
        laterBtn.onclick = () => hideOtaUpdateModal();
    }
    modal.style.display = 'flex';
}

/** Neuer Code am Büro-Laptop (officeBundleId), webVersion kann gleich bleiben. */
function showOtaOfficeBundleModal(installedVer, officeBase, bundleMeta) {
    const modal = document.getElementById('ota-update-modal');
    const office = normalizeOtaBase(officeBase);
    if (!modal || !office) {
        loadWlanUpdateNow();
    ...
    const laterBtn = document.getElementById('ota-update-later');
    const iconEl = modal.querySelector('.ota-update-icon');
    if (h2) h2.textContent = 'Neuer Stand am Laptop';
    if (iconEl) iconEl.textContent = '💻';
    if (textEl) {
        textEl.innerHTML = 'Am Büro-Server liegt ein <strong>neuerer Programmstand</strong> (weiterhin v'
            + installedVer + ').'
            + '<br><br>Das passiert wenn du am Laptop etwas geändert hast — <strong>ohne Versionsnummer +1</strong>.';
    }
    if (confirmBtn) {
        confirmBtn.textContent = 'Jetzt vom Büro-Server laden';
        confirmBtn.style.background = '#25D366';
        confirmBtn.disabled = false;
        confirmBtn.onclick = () => {
            hideOtaUpdateModal();
            ...
            AppStorage.remove('ota_banner_dismissed');
            AppStorage.setRaw('ota_web_base_url', office);
            if (bundleMeta?.officeBundleId) recordOfficeBundleId(bundleMeta.officeBundleId);
            navigateToOtaUrl(office + '/index.html?t=' + Date.now());
        };
    }
    if (laterBtn) {
        ...
        laterBtn.style.display = '';
        laterBtn.onclick = () => {
            AppStorage.setRaw('ota_office_bundle_dismissed', String(bundleMeta?.officeBundleId || ''));
            hideOtaUpdateModal();
        };
    }
    modal.style.display = 'flex';
    setUpdateAvailableUI(true, installedVer);
}
...
async function checkForWebUpdate(manual) {
    ...
    if (remoteVer <= installedVer) {
        const { office } = await getOtaConfigBases();
        const officeMeta = office ? await officeHasNewerBundle(office) : null;
        const bundleDismissed = String(AppStorage.getRaw('ota_office_bundle_dismissed') || '');
        if (officeMeta && officeMeta.officeBundleId !== bundleDismissed && manual) {
            showOtaOfficeBundleModal(installedVer, office, officeMeta);
            return;
        }
        hideOtaUpdateModal();
        setUpdateAvailableUI(false);
        pendingOtaUpdate = null;
        if (manual) {
            if (office && await probeOtaBase(office)) {
                showOtaUpToDateWithOfficeOption(installedVer, office);
            } else {
                showOtaInfoModal(
                    'App ist aktuell',
                    'Version <strong>v' + installedVer + '</strong> (GitHub).'
                        + '<br><br>Büro-PC nicht erreichbar — <code>START.bat</code> starten und gleiches WLAN prüfen.',
                    '✓'
                );
            }
        }
...
/** Warenlinien-Dropdown: fehlt im alten HTML, wird per JS ergänzt — Inhalt kommt nur aus der Cloud. */
function appSupplierLinesFromDb(supplierName) {
    ...
    let lines = null;
    if (typeof lookupSupplierLines === 'function') {
        lines = lookupSupplierLines(db.supplierLines, name, db.supplierLinesCleared);
    } else if (db.supplierLines) {
        lines = db.supplierLines[name];
        if (!lines) {
            Object.keys(db.supplierLines).forEach((k) => {
                try {
                    if (decodeURIComponent(k) === name) lines = db.supplierLines[k];
                } catch (_) {}
            });
        }
    }
...
function ensureWarenlinieUi() {
    const sup = document.getElementById('supplierSelect');
    if (sup && !document.getElementById('supplierLineWrap')) {
        const wrap = document.createElement('div');
        wrap.id = 'supplierLineWrap';
        wrap.style.cssText = 'display:none; margin-bottom:10px;';
        wrap.innerHTML = '<label>Warenlinie (optional):</label>'
            + '<select id="supplierLineSelect" style="width:100%; padding:12px; border-radius:8px; border:1px solid #ccc; font-size:16px;"></select>';
        const anchor = sup.nextSibling;
        if (anchor) sup.parentNode.insertBefore(wrap, anchor);
        else sup.parentNode.appendChild(wrap);
    }
    if (sup && !sup.getAttribute('onchange')) sup.setAttribute('onchange', 'refreshWarenlinieSelects()');

    const lief = document.getElementById('lief');
    if (lief && !document.getElementById('lkw-supplier-line-wrap')) {
        const field = document.createElement('div');
        field.className = 'field';
        field.id = 'lkw-supplier-line-wrap';
        field.style.display = 'none';
        field.innerHTML = '<label>Warenlinie (optional)</label><select id="lkw-supplier-line"></select>';
        const liefField = lief.closest('.field') || lief.parentElement;
        if (liefField && liefField.parentNode) liefField.parentNode.insertBefore(field, liefField.nextSibling);
    }
    if (lief && !lief.getAttribute('onchange')) lief.setAttribute('onchange', 'refreshWarenlinieSelects()');
}

function fillSupplierLineSelect(supplierName, wrapId, selectId) {
    const wrap = document.getElementById(wrapId);
    const sel = document.getElementById(selectId);
    if (!wrap || !sel) return;
    const lines = appSupplierLinesFromDb(supplierName);
    if (!lines.length) {
    ...
    sel.innerHTML = '<option value="">— Gemischt / optional —</option>'
        + lines.map((l) => `<option value="${String(l).replace(/"/g, '&quot;')}">${l}</option>`).join('');
}

function refreshWarenlinieSelects() {
    ensureWarenlinieUi();
    const sup = document.getElementById('supplierSelect');
    const lief = document.getElementById('lief');
    if (typeof updateSupplierLineSelect === 'function') updateSupplierLineSelect();
    else if (sup) fillSupplierLineSelect(sup.value, 'supplierLineWrap', 'supplierLineSelect');
    if (typeof updateLkwSupplierLineSelect === 'function') updateLkwSupplierLineSelect();
    else if (lief) fillSupplierLineSelect(lief.value, 'lkw-supplier-line-wrap', 'lkw-supplier-line');
}
window.refreshWarenlinieSelects = refreshWarenlinieSelects;
...
function getLogistikFullCloudPayload() {
    ...
    const payload = {
        ...
        deliveries: typeof deliveriesFuerCloudSync === 'function' ? deliveriesFuerCloudSync(lData.deliveries || []) : (lData.deliveries || []),
...
function applyLogistikFullFromCloud(data) {
    ...
    if (data.deliveries != null) {
        const merged = typeof mergeLieferungenCloudMitLokalLkw === 'function'
            ? mergeLieferungenCloudMitLokalLkw(lDb.deliveries || [], data.deliveries)
            : mergeLieferungen(lDb.deliveries || [], data.deliveries);
        const before = JSON.stringify(lDb.deliveries || []);
        if (JSON.stringify(merged) !== before) {
            lDb.deliveries = merged;
            changed = true;
        }
    }
    setArr('checklistMorningTemplate');
    if (data.dailyStaff && typeof data.dailyStaff === 'object') {
        lDb.dailyStaff = { ...data.dailyStaff, ...(lDb.dailyStaff || {}) };
        changed = true;
    }
    if (data.dailyAttendance && typeof data.dailyAttendance === 'object') {
        lDb.dailyAttendance = { ...data.dailyAttendance, ...(lDb.dailyAttendance || {}) };
        changed = true;
    }
    setObj('workerColors');
    setObj('teamDayBrief');
    if (data.artikelMarkt && typeof data.artikelMarkt === 'object' && !Array.isArray(data.artikelMarkt)) {
        lDb.artikelMarkt = { ...(lDb.artikelMarkt || {}), ...data.artikelMarkt };
        changed = true;
    }
    if (data.teamTagesMengen && typeof data.teamTagesMengen === 'object' && !Array.isArray(data.teamTagesMengen)) {
        lDb.teamTagesMengen = mergeTeamTagesMengen(lDb.teamTagesMengen || {}, data.teamTagesMengen);
        if (!lDb.dailyStaff) lDb.dailyStaff = {};
        syncDailyStaffAusTeam(lDb.teamTagesMengen, lDb.dailyStaff);
        changed = true;
    }
    if (Array.isArray(data.teamSortierBuchungen)) {
        const lokalHat = (lDb.teamSortierBuchungen || []).length > 0;
        const cloudLeer = data.teamSortierBuchungen.length === 0;
        if (!(cloudLeer && lokalHat)) {
            lDb.teamSortierBuchungen = mergeTeamSortierBuchungen(
    ...
    if (Array.isArray(data.deletedSortierBuchungen)) {
        lDb.deletedSortierBuchungen = mergeDeletedSortierKeys(lDb.deletedSortierBuchungen, data.deletedSortierBuchungen);
        changed = true;
    }
    if (typeof bereinigeGeloeschteSortierDeliveries === 'function') bereinigeGeloeschteSortierDeliveries(lDb);
    if (typeof ensureSortierenDeliveries === 'function') ensureSortierenDeliveries(lDb);
    if (Array.isArray(data.supplierLinesCleared)) {
        lDb.supplierLinesCleared = typeof mergeSupplierLinesClearedLists === 'function'
            ? mergeSupplierLinesClearedLists(lDb.supplierLinesCleared, data.supplierLinesCleared)
            : [...new Set([...(lDb.supplierLinesCleared || []), ...data.supplierLinesCleared])];
        changed = true;
    }
    if (data.supplierLines && typeof data.supplierLines === 'object') {
        const before = JSON.stringify(lDb.supplierLines || {});
        lDb.supplierLines = typeof mergeSupplierLinesForSync === 'function'
...
function safeLogistikCloudPatchPayload(raw) {
    ...
    if (!Array.isArray(payload.deletedSortierBuchungen) || payload.deletedSortierBuchungen.length === 0) {
        delete payload.deletedSortierBuchungen;
    }
...
async function pushSortierNachDruckSilent() {
    ...
    const localBuch = typeof asSortierBuchungenArray === 'function'
        ...
        : (Array.isArray(lData.teamSortierBuchungen) ? lData.teamSortierBuchungen : []);
    if (!localBuch.length) return;

    let mergedBuch = localBuch;
    let mergedDel = lData.deletedSortierBuchungen || [];
    let mergedMengen = lData.teamTagesMengen || {};
    let mergedDeliveries = typeof deliveriesFuerCloudSync === 'function'
        ? deliveriesFuerCloudSync(lData.deliveries || [])
        : (lData.deliveries || []).filter((d) => d && d.source === 'sortieren');
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (cloud) {
                ...
                if (typeof mergeLieferungenCloudMitLokalLkw === 'function' && cloud.deliveries != null) {
                    mergedDeliveries = deliveriesFuerCloudSync(
                        mergeLieferungenCloudMitLokalLkw(lData.deliveries || [], cloud.deliveries)
                    );
                }
            }
        }
    } catch (_) { /* offline merge */ }

    const patch = {
        teamSortierBuchungen: mergedBuch,
        deletedSortierBuchungen: mergedDel,
...
async function silentPushLogistikToCloud() {
    ...
    let payload = getLogistikFullCloudPayload();
    try {
        const res = await cloudFetch(APP_CONFIG.CLOUD_URL + ".json?t=" + Date.now());
        if (res.ok) {
            const cloud = await res.json();
            if (cloud && typeof mergeLogistikPayloadMitCloud === 'function') {
                payload = mergeLogistikPayloadMitCloud(payload, cloud);
            }
        }
    } catch (e) { /* offline — nur lokales Payload */ }
    cloudFetch(APP_CONFIG.CLOUD_URL + ".json", {
        method: 'PATCH',
        body: JSON.stringify(safeLogistikCloudPatchPayload(payload)),
        headers: { 'Content-Type': 'application/json' }
    })
    .then(res => {
    ...
    setTimeout(() => {
        ...
        try {
            ...
            if (typeof AndroidApp !== 'undefined' && AndroidApp.printPage) {
                const docHtml = typeof buildPrintDocumentHtml === 'function'
                    ? buildPrintDocumentHtml(title, '', content, false)
                    : content;
                if (typeof printHtmlViaIframe === 'function' && printHtmlViaIframe(docHtml, restoreDark)) {
                    return;
                }
